...
from path_utils import PATH_TO_DATASPLITS, PATH_TO_SPIKE_REGRESS_DATA_PASSIVE
...
def remove_outlier(y_hat):
    """
    Remove outliers in predicted data.
    :param y_hat:
    :return:
    """
    mean_pp = np.mean(y_hat)
    std_pp = np.std(y_hat)
    ind_pp = np.where(np.abs(y_hat - mean_pp) > std_pp * 5)
    for ind in ind_pp[0]:
        if ind - 2 < 0:
            ind_init = 0
        else:
        ...
        if ind + 2 > len(y_hat):
            ...
            ind_end = ind + 2
        value_tmp = np.median(np.concatenate((y_hat[ind_init:ind], y_hat[ind + 1:ind_end]), 0))
...
def make_layer_predictions(layer_activations, spike_counts, mvt_durations, monkey_name, session_date,
                           ...
                           plot_pred=False,shuffle_flag=False,active_as_passive_flag=False):
    """
    ...
    if active_as_passive_flag:
        ...
        PATH_TO_DATASPLITS_passive = PATH_TO_SPIKE_REGRESS_DATA_PASSIVE
        idx_trials_path = os.path.join(PATH_TO_DATASPLITS_passive,
                                   'idx_split_{}_passive.p'.format(monkey_name))

        all_indexes = pd.read_pickle(idx_trials_path)
        train_ids_pas = all_indexes['train_idx']
        test_ids_pas = all_indexes['test_idx']
        n_trials_act_pas_tr = len(train_ids_pas)
        n_trials_act_pas_te = len(test_ids_pas)
    ...
    if active_as_passive_flag:
        train_ids_act = rng.choice(train_ids_act,n_trials_act_pas_tr,replace=False)
    ...
    if active_as_passive_flag:
        test_ids_act = rng.choice(test_ids_act,n_trials_act_pas_te,replace=False)
    ...
    x_train, y_train, new_mvt_duration_tr = compute_regression_rates(layer_activations_tr,
    ...
    x_test, y_test, new_mvt_duration_te = compute_regression_rates(layer_activations_te,
    ...
    for idx_mvt,t in enumerate(train_ids_act):
        if active_as_passive_flag:
            idx_trial_train_concat.extend(new_mvt_duration_tr[idx_mvt]*[t])
        else:
