...
conda activate DeepProprio_check
...
for data in Snap,20190829 Butter,20180326 Lando,20170917 Han,20171122 Chips,20170913 S1Lando,20170917; do set -- $data;
  ...
  for id in 8030 #5015
    do
    echo "Experiment" $id
    python3.8 compute_session_predictivity_h5_cv_pool_align.py --monkey $1 --session $2 --exp $id --active_start "mvt" --active_length 0 \
     --align 100 --permut_t --permut_m  --constant_input --path_to_act "$PATH_TO_ACTIVATIONS/active/" --path_to_results "$PATH_TO_PREDICTIONS/active/"
  done
done
IFS=$OLDIFS
IFS=$OLDIFS
