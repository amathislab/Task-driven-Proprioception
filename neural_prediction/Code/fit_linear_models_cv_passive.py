...
from path_utils import PATH_TO_NEURAL_DATA, PATH_TO_SPIKE_REGRESS_DATA_PASSIVE, PATH_TO_MATLAB_DATA, PATH_TO_SAVE_LINEAR
...
def main(monkey_name, session_date, active_start, is_short):
    ...
    path_to_save_linear_folder = os.path.join(PATH_TO_SAVE_LINEAR,'passive','linear_models')
    ...
    idx_passive = np.where(~np.isnan(np.array(data_df.bumpDir)))[0]
    data_df_passive = data_df.iloc[idx_passive]
    mvt_duration_all = np.array(data_df_passive.idx_endTime) - np.array(data_df_passive.idx_movement_on)
    ...
    # Passive
    rates_act_start_end = session_sets.PassiveSets.rates_trial
    dir_act_start_end_trial = session_sets.PassiveSets.dir_trial
    rates_t_act_start_end = session_sets.PassiveSets.rates_t
    dir_act_start_end = session_sets.PassiveSets.dir
    pos_act = session_sets.PassiveSets.pos
    pos_act_norm = session_sets.PassiveSets.pos_norm
    vel_act = session_sets.PassiveSets.vel
    vel_act_norm = session_sets.PassiveSets.vel_norm
    acc_act = session_sets.PassiveSets.acc
    acc_act_norm = session_sets.PassiveSets.acc_norm
    force_act = session_sets.PassiveSets.force
    force_act_norm = session_sets.PassiveSets.force_norm

    m_len_act_start_end = session_sets.PassiveSets.m_len
    m_vel_act_start_end = session_sets.PassiveSets.m_vel

    j_ang_act_start_end = []
    j_vel_act_start_end = []
    bump_start = np.array(data_df_passive['idx_bumpTime'],dtype=np.int)
    for ii in range(len(data_df_passive)):
        ## Start of passive bump
        bump_start_tmp = bump_start[ii]

        ## Take joint angles and compute velocity
        j_tmp = np.array(data_df_passive.iloc[ii]['opensim'][:,:4])
        j_vel_tmp = np.gradient(j_tmp,0.01,axis=0)

        ## Take only passive bump
        j_tmp = j_tmp[bump_start_tmp:bump_start_tmp+13,:]
        j_vel_tmp = j_vel_tmp[bump_start_tmp:bump_start_tmp+13,:]
        j_ang_act_start_end.append(j_tmp)
        j_vel_act_start_end.append(j_vel_tmp)

    if monkey_name == 'Chips':
        PATH_TO_DATA_pp = PATH_TO_NEURAL_DATA
        ...
        datasets = load_monkey_datasets(PATH_TO_DATA_pp,
                                            monkey_name,
                                            session_date,
                                            True,
                                            active_start=active_start,
        ...
        m_len_vel_start_end_all = datasets['passive']['muscle_coords'].copy()
    ...
    del data_df, data_df_passive, session_sets
    ...
    bad_trials_path = os.path.join(PATH_TO_SPIKE_REGRESS_DATA_PASSIVE,
                                    'excludedtrials_{}_{}_passive.p'.format(monkey_name, session_date))
    excluded_trials = pd.read_pickle(bad_trials_path)
    excluded_trials_ids = np.array(excluded_trials['bad_trials'])
    ...
    ### For Butter since in the predictions are already removed, delete them before
    if monkey_name == 'Butter':
        mvt_duration_all = [mvt_duration_all[ii] for ii in range(len(mvt_duration_all)) if ii not in excluded_trials_ids]

        array_list_new = []
        for array in array_list:
            array = [array[ii] for ii in range(len(array)) if ii not in excluded_trials_ids]
            array_list_new.append(array)
        ...
        if 'active' in kin_var:
            ...
            for t in range(len(kin_array)):
                if t in train_ids_act:
                    if len(kin_array[0].shape) == 1:
                        if kin_array[0].shape[0] >= 2:
                            kin_array_concat_tr.extend(np.asarray(np.hstack(kin_array[t])))
                        else:
                            kin_array_concat_tr.extend(np.asarray(np.vstack(kin_array[t])))
                    else:
                        if kin_array[0].shape[0] >= 2:
                            kin_array_concat_tr.extend(np.asarray(np.vstack(kin_array[t])))
                        else:
                    ...
                    if len(kin_array[0].shape) == 1:
                        if kin_array[0].shape[0] >= 2:
                            ...
                            kin_array_concat_te.extend(np.asarray(np.vstack(kin_array[t])))
                    else:
                        if kin_array[0].shape[0] >= 2:
                            kin_array_concat_te.extend(np.asarray(np.vstack(kin_array[t])))
                        else:
    ...
    array_list_act = [np.stack((pos_act, vel_act), axis=-1),
                      ...
                      np.concatenate([np.stack(j_ang_act_start_end), np.stack(j_vel_act_start_end), np.stack(m_len_act_start_end), np.stack(m_vel_act_start_end)],axis=-1)
                      ]
    ...
    if monkey_name == 'Butter':
        array_list_new = []
        for array in array_list_act:
            array = [array[ii] for ii in range(len(array)) if ii not in excluded_trials_ids]
            array_list_new.append(array)

        array_list_act = array_list_new
    ...
    for kin_array, kin_var in zip(array_list_multi, kin_vars_regression_multi):
        ...
        kin_array = np.asarray(kin_array)
        kin_array = np.reshape(kin_array,[kin_array.shape[0], kin_array.shape[1],-1])
        # DATA PREPARATION
...
if __name__ == '__main__':
    ...
    parser.add_argument('--monkey', type=str, help='Which monkey?', required=False, default='Snap') #Chips,20170913  Snap,20190829
    parser.add_argument('--session', type=int, help='Which session data?', required=False, default=20190829)
    parser.add_argument('--start', type=str, help='Which active start?', required=False, default='mvt')
    parser.add_argument('--short', type=str, help='Short active?', required=False, default='n')
