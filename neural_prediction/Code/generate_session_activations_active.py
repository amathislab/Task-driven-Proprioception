...
def main(monkey_name, session_date, exp_id, train_iter, active_start='mvt', active_length=0, align=100,
         permut_m=False, permut_t=False, constant_input=False, n_pca=25, input_key='muscle_coords', path_to_act='..'):
    ...
    for model_name in model_list:
        ...
        for task_data, task_type in zip(task_datasets, selected_tasks):
            ...
            elif PCR_task in ['classification', 'bt', 'untrained']: # (PCR_task == 'classification') or (PCR_task == 'bt'):
                if 'lstm' not in model_name:
                    if int(float(model_config['nlayers'])) < 5:
            ...
            with graph.as_default():
                ...
                if (PCR_task in regression_task_list) or (PCR_task in ['bt','untrained']):
            ...
            with graph.as_default():
                with tf.Session(config=myconfig) as sess:
                    ...
                    for step in range(num_steps):
                        # Next input batches
                        batch_x = task_data[input_key][batch_size * step:batch_size * (step + 1)]#*100
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_key', type=str, help='Which input(muscle_coords or joint_coords)?', required=False, default = 'muscle_coords')
    ...
    main(args.monkey, args.session, args.exp_id, args.train_iter,
         ...
         args.permut_m, args.permut_t, args.constant_input, args.n_pca, args.input_key, args.path_to_act)