...
for data in Snap,20190829 Butter,20180326 Lando,20170917 Han,20171122 Chips,20170913 S1Lando,20170917; do set -- $data;
  ...
  for id in 4015
    do
    echo "Experiment" $id
    python3.6 generate_session_activations_active.py --monkey $1 --session $2 --exp $id \
        --active_start "mvt" --active_length 0 \
        --align 100 --permut_t --permut_m  --constant_input --n_pca 75 --path_to_act "$PATH_TO_ACTIVATIONS/active"
        # --train_iter 0
  done
done
IFS=$OLDIFS
