...
def translate_output(output, latency):
    """Translate the output in time of a specific latency (1 time point = 10 ms) for forward prediction.
    Args:
        output (numpy): The output to translate. [batch_size x time x n_features]
        latency (int): Latency to translate the output in time

    Returns:
        numpy: Same as output but translated in time
    """
    mytraj = np.zeros_like(output)

    mytraj[:,:,:-latency] = output[:,:,latency:].copy()
    mytraj[:,:,-latency:] = np.expand_dims(output[:,:,-1],2).copy()
...
class RDataset():
    ...
    def next_trainbatch(self, batch_size, step=0):
        ...
        batch_size, n_inputs,n_time,n_channels = mybatch_targets.shape

        mybatch_targets = np.transpose(mybatch_targets, [0,1,3,2])
        mybatch_targets = np.reshape(mybatch_targets, [batch_size, -1, n_time])
...
class RTrainer:
    ...
    def __init__(self, model=None, dataset=None, test_dataset=None, global_step=None):
        ...
        self.validation_accuracy = 0
                ## Add variables for normalization
    ...
    def build_graph(self, **kwargs):
        ...
        _, ninputs, ntime, n_channels = self.dataset.train_data.shape
        ...
        with tf.Graph().as_default() as self.graph:
            ...
            self.y = tf.placeholder(tf.float32, shape=[self.batch_size, nouttime, ninputs*n_channels], name="y") # ALWAYS endeffectors for now.
            ...
            pred = tf.reshape(scores_eval, [-1, ninputs*n_channels])  #3
            targ = tf.reshape(self.y, [-1, ninputs*n_channels])   #3
