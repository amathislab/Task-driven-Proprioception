...
class Dataset():
    ...
    def __init__(self, path_to_data=None, path_to_val=None, data=None, dataset_type='train', key='spindle_firing', target_key='torque_coords', reach_key='target_coords', fraction=None):
    ...
    def make_data(self, mydata):
        ...
        if self.path_to_data is not None:
            ...
            if self.dataset_type == 'train':
                ...
                self.train_reach = datafile[self.reach_key]
                ...
                self.val_targets = datafile_val[self.target_key]
                self.val_reach = datafile[self.reach_key]
            elif self.dataset_type == 'test':
                self.test_data = datafile[self.key]
                self.test_targets = datafile[self.target_key]
                self.test_reach = datafile[self.reach_key]
    ...
    def next_trainbatch(self, batch_size, step=0, normalize = False):
        ...
        mybatch_targets = self.train_targets[self.shuffle_idx[step]:self.shuffle_idx[step]+batch_size]
        mybatch_reach = self.train_reach[self.shuffle_idx[step]:self.shuffle_idx[step]+batch_size]

        mybatch_targets = set_kin_dimensions(mybatch_targets, self.n_out_time)
        mybatch_reach = set_kin_dimensions(mybatch_reach, self.n_out_time)

        return (mybatch_data, mybatch_targets, mybatch_reach)
    ...
    def next_valbatch(self, batch_size, type='val', step=0, normalize = False):
        ...
        if type == 'train':
            ...
            mybatch_reach = self.train_reach[batch_size*step:batch_size*(step+1)]
        elif type == 'val':
            ...
            mybatch_reach = self.val_reach[batch_size*step:batch_size*(step+1)]
        elif type == 'test':
            ...
            mybatch_reach = self.test_reach[batch_size*step:batch_size*(step+1)]
            
        mybatch_data = mybatch_data.astype('float32') 
        mybatch_targets = set_kin_dimensions(mybatch_targets, self.n_out_time) 
        mybatch_reach = set_kin_dimensions(mybatch_reach, self.n_out_time)
...
class Trainer:
    ...
    def build_graph(self, **kwargs):
        ...
        _, ncoords_target, _ = self.dataset.train_reach.shape
        ...
        with tf.Graph().as_default() as self.graph:
            ...
            self.target = tf.placeholder(tf.float32, shape=[self.batch_size, nouttime, ncoords_target], name="target")

            # Set up optimizer, compute and apply gradients
            scores, _ = self.model.predict(self.X, self.target, is_training=True)
            ...
            # Calculate metrics
            scores_eval, _ = self.model.predict(self.X, self.target, is_training=False)
...
#             self.load_step(0, log_dir = log_dir)
        ...
        for self.global_step in range(max_iter):
            ...
            # Training step
            batch_X, batch_y, batch_target = self.dataset.next_trainbatch(
        ...
        ### Test the network
        test_accuracy, train_accuracy = self.test_model()
        train_params['train_accuracy'] = float(train_accuracy)
    ...
    def eval(self):
        ...
        for i in range(num_iter):
            batch_X, batch_y, batch_target = self.dataset.next_valbatch(self.batch_size, step=i)
            feed_dict = {self.X: batch_X - self.train_data_mean, self.y: batch_y, self.target: batch_target}
    ...
    def test_model(self):
        ...
        acc_test = []
        for i in range(num_iter):
            batch_X, batch_y, batch_target = self.test_dataset.next_valbatch(self.batch_size, 'test', step=i)
        ...
        ### TRAINING SET
        num_iter = self.dataset.train_data.shape[0] // self.batch_size
        acc_train = []
        for i in range(num_iter):
            batch_X, batch_y, batch_target = self.dataset.next_valbatch(self.batch_size, 'train', step=i)
